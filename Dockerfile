# Globus — single-stage Python image, ~120MB on top of python:3.12-slim.
...
RUN groupadd -g ${GLOBUS_GID} globus \
    && useradd  -u ${GLOBUS_UID} -g globus -d /app -s /bin/bash globus
...
COPY --chown=globus:globus members/  /app/members/
COPY --chown=globus:globus docs/     /app/docs/
COPY --chown=globus:globus docker/entrypoint.sh /app/docker/entrypoint.sh
RUN chmod 0755 /app/docker/entrypoint.sh /app/scripts/run_agent.py

# Persistent state lives in two volumes so re-creating the container
# (e.g. on image upgrade) keeps briefs + Drive-cached files intact.
#   /var/lib/globus/agents   — agent_runner brief output dir
#   /var/lib/globus/raw-data — google_drive disk cache
# /app/.state is a small dir for SESSION_SECRET (auto-generated on
# first boot if not provided via env).
RUN mkdir -p /var/lib/globus/agents /var/lib/globus/raw-data /app/.state \
    && chown -R globus:globus /var/lib/globus /app/.state
VOLUME ["/var/lib/globus/agents", "/var/lib/globus/raw-data"]
